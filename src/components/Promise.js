...
const Promise = (props) => {
    ...
    return(<div className="promise">
        <img className="promiseImage" src={images[props.src]} alt={props.src} unselectable={true} draggable={false}/>
        <h5 className="title">{props.title}</h5>
        <div className="desc">{props.desc}</div>
        <div className="groupKnowMore">
            <h6 className="knowMore">Know More</h6>
            <img className="pinkArrow" src={pinkArrow} alt="pinkArrow" unselectable={true} draggable={false}/>
        </div>
    </div>)
};
